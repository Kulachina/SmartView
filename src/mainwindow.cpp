...
QWidget* MainWindow::CreateTabMasterLAS(){
    ...
    connect(btn_saveas,&QPushButton::clicked, this,&MainWindow::CreateAllDocLAS);
    ...
    connect(ch_sel_1_table_las_, &QCheckBox::toggled, this,[this,combo_name_2](){
        if(ch_sel_1_table_las_->isChecked()){
            flag_termocompens_ = true;
        }else {
    ...
    connect(check_step_bar_las_, &QCheckBox::clicked, this,[=](){
        if(check_step_bar_las_->isChecked()){
            s_step_bar_las_->setEnabled(true);
        } else {
            s_step_bar_las_->setEnabled(false);
        }
    });
...
void MainWindow::CreateInContentLAS(){
    tab_las_->clear();
    for(auto& data : data_base_.GetDataSerACM()){
        QVector<Canal>& acm = data.vec_canal;
...
void MainWindow::FilingTableLAS(QStandardItemModel* model){
...
void MainWindow::FillAllTablesLAS(){
    if(!data_base_.GetDataSerACM().isEmpty()){
        for(DataSeriesSensor& data : data_base_.GetDataSerACM()){
            QVector<Canal>& acm = data.vec_canal;
...
void MainWindow::AnalisingSeriesLAS(DataSeriesSensor data){
    QVector<Canal>& acm = data.vec_canal;
...
QWidget* MainWindow::CreateTabMasterACM(){
...
void MainWindow::WindowMasterPoint(){
    if(!first_open_3_){
        ...
        QVBoxLayout *vbox_master = new QVBoxLayout(window_c_p_);
        QTabWidget *tab_master = new QTabWidget(window_c_p_);
        ...
        window_c_p_->setLayout(vbox_master);
        first_open_3_ = true;
    }
    CreateInContentLAS();
    CreateInContentACM();
...
void MainWindow::CreateInContentACM(){
    tab_->clear();
    for(auto& data : data_base_.GetDataSerACM()){
        QVector<Canal>& acm = data.vec_canal;
        QWidget *tab_sensor = new QWidget();
...
void MainWindow::CreateAllDocLAS(){
...
void MainWindow::WindowSensorAndCanal(QVector<DataSeriesSensor>& vec_data){
    if (first_open_win_sens_can_){
        ...
        connect(btn_select, &QPushButton::clicked, this, &MainWindow::CheckCanals);
        connect(btn_unselect, &QPushButton::clicked, this, &MainWindow::UncheckCanals);
...
DataSeriesSensor* MainWindow::PreparationData(){
    if(!sensor_list_->selectedItems().isEmpty()){
        QString sensor_name = sensor_list_->selectedItems().first()->text();
